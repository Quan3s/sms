...
// HÀM TỰ ĐỘNG CẬP NHẬT CẤU TRÚC BẢNG (MIGRATION)
const initDB = async () => {
    try {
        ...
        // TỰ ĐỘNG THÊM CỘT NẾU BẢNG CŨ THIẾU (Sửa lỗi bạn vừa gặp)
        await pool.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS display_name TEXT;");
        await pool.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar_color TEXT DEFAULT '#22d3ee';");
        ...
        console.log("✅ Database đã được đồng bộ cấu trúc mới nhất.");
    } catch (e) {
        console.error("❌ Lỗi DB khởi tạo:", e.message);
    }
};
initDB();
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        } else res.status(401).json({ error: "Tài khoản hoặc mật khẩu không đúng!" });
...
app.get('/api/history/:room', async (req, res) => {
    try {
        const r = await pool.query(`
            ...
            WHERE m.room_id = $1 
            ORDER BY m.created_at ASC LIMIT 100`, [req.params.room]);
        res.json(r.rows);
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, sans-serif; }
        ...
        .msg-list { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 14px; }
        .bubble { max-width: 80%; padding: 12px 16px; border-radius: 20px; font-size: 15px; }
        .me { align-self: flex-end; background: var(--p); color: #000; border-bottom-right-radius: 2px; }
        ...
        .avatar { width: 48px; height: 48px; border-radius: 15px; display: flex; align-items: center; justify-content: center; font-weight: bold; color: #000; position: relative; }
        .online-dot { width: 12px; height: 12px; border-radius: 50%; background: #22c55e; border: 2px solid #1e293b; position: absolute; bottom: -2px; right: -2px; display: none; }
        .online-dot.on { display: block; }
    </style>
</head>
<body>
    <div id="auth-screen" class="screen active">
        <div style="margin: auto; width: 90%; max-width: 380px; background: var(--card); padding: 30px; border-radius: 25px; text-align: center;">
            <h1 style="color:var(--p)">Z-Chat</h1><br>
            <div id="l-box">
                ...
                <p style="margin-top:20px; font-size:14px; opacity:0.6" onclick="toggleAuth(true)">Chưa có tài khoản? Đăng ký</p>
            </div>
            <div id="r-box" style="display:none">
                <input id="ru" placeholder="Tên đăng nhập (viết liền)">
                <input id="rn" placeholder="Tên hiển thị">
                <input id="rp" type="password" placeholder="Mật khẩu">
                <button class="btn" style="width:100%" onclick="handleAuth('register')">Đăng ký</button>
                <p style="margin-top:20px; font-size:14px; opacity:0.6" onclick="toggleAuth(false)">Đã có tài khoản? Đăng nhập</p>
            </div>
        </div>
    </div>
    ...
    <div id="app-screen" class="screen">
        <div class="header">
            <b>Z-CHAT PRO</b>
            <div style="background:#334155; padding:5px 12px; border-radius:15px; font-size:12px">
                <span id="me-n"></span> <b id="me-i" style="color:var(--p)"></b>
            </div>
        </div>
        <div id="tab-home" style="flex:1; overflow-y:auto">
            <div style="background:#1e293b; padding:15px; border-radius:15px; margin:15px; display:flex; justify-content:space-between" onclick="goRoom('global','Cộng Đồng')">
                <b>🌍 Phòng Toàn Cầu</b><span>→</span>
            </div>
        ...
        <div class="nav">
            ...
            <div class="nav-tab" id="ns" onclick="switchT('search', this)">🔍<br>Tìm</div>
            <div class="nav-tab" onclick="logout()">🚪<br>Thoát</div>
        </div>
    </div>

    <div id="chat-view" class="screen" style="position:fixed; inset:0; background:var(--bg); z-index:100">
        <div class="header">
            <button onclick="back()" style="background:none; border:none; color:white; font-size:24px">✕</button>
            <b id="chat-title"></b><div style="width:30px"></div>
        </div>
        ...
        <div style="padding:15px; display:flex; gap:10px; background:#1e293b">
            <input id="mi" placeholder="Nhập tin nhắn..." style="margin:0" autocomplete="off" oninput="isT()">
    ...
    <script>
        const socket = io();
        let me = JSON.parse(localStorage.getItem('z_v8_user'));
        let cr = '';
        ...
        async function handleAuth(type) {
            ...
            try {
                const res = await fetch('/api/auth/'+type, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(b)});
                const d = await res.json();
                if(d.success) {
                    if(isL) { localStorage.setItem('z_v8_user', JSON.stringify(d.user)); me=d.user; start(); }
                    else { alert(\"Đăng ký xong!\"); toggleAuth(false); }
                } else alert(d.error);
        ...
        async function search() {
            ...
            document.getElementById('search-results').innerHTML = us.map(u => \`
                <div style=\"background:var(--card); padding:15px; border-radius:15px; margin-bottom:10px; display:flex; align-items:center; gap:12px\" onclick=\"goPrivate('\${u.num_id}', '\${u.display_name}')\">
...
server.listen(PORT, () => console.log('🚀 Z-Chat Pro V8.1 sẵn sàng trên cổng ' + PORT));
